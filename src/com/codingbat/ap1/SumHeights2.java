...
start=2, end=4 yields a sum of 1*2 + 5 = 7. The start end end index will both be valid indexes into the array with
start <= end.
...
public class SumHeights2 {
    public int sumHeights2(int[] heights, int start, int end) {
        ...
        for (int i = start; i < end; i++) {
            ...
            if (diff < 0) { //previous < following -> increases in height count double
                result += Math.abs(diff) * 2;
            } else {
                result += diff;
            }
        }
